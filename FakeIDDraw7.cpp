...
LRESULT CALLBACK NewWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (g_bRawMouseInputFix2 && uMsg == WM_INPUT)
	{
		ProcessRawMouseInput(lParam, g_lRMILastX, g_lRMILastY);
	}
	
	if (uMsg == WM_ACTIVATEAPP && g_bDontShutdownRenderer && g_bWindowed)
	{
		return TRUE;
	}
	else
...
ULONG   __stdcall FakeIDDraw7::Release(void)
{
	...
	if (dwCount == 0) 
	{
		logf(this, LDEBUG, "FakeIDDraw7::Release - release success");
		m_pIDDraw = NULL;		
		delete(this); 
	}
	else
	{
		logf(this, LERROR, "FakeIDDraw7::Release - release failed, reference count = %d", dwCount);
		m_pIDDraw = NULL;		
		delete(this); 
	}
...
{		
	...
	{
		*lplpDDSurface = new FakeIDDrawSurface7(*lplpDDSurface);
	}
...
HRESULT  __stdcall FakeIDDraw7::SetCooperativeLevel(HWND hWnd, DWORD dwFlags)
{
	...
	if (!g_bWindowHooked /*&& g_dwWindowed*/)
	{
		...
		if (lResult)
		{
			...
			logf(this, LINFO, "WindowProc hook set! (%08X -> %08X)", lResult, (LONG)NewWindowProc);
		}
		else
		{
			logf(this, LERROR, "Could not set WindowProc hook!");
		}
...
HRESULT  __stdcall FakeIDDraw7::SetDisplayMode(DWORD a, DWORD b, DWORD c, DWORD d, DWORD e)
{
	g_dwWidth = a;
	g_dwHeight = b;
	logf(this, LDEBUG, "FakeIDDraw7::SetDisplayMode - %d x %d", g_dwWidth, g_dwHeight);
